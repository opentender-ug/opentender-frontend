...
import {
	IApiResultGeoJSON,
	IApiResultAuthority,
	IApiResultAuthoritySimilar,
	IApiResultCompany,
	IApiResultCompanySimilar,
	IApiResultDownloadTenderSearch,
	IApiResultNuts,
	IApiResultPortalsStats,
	IApiResultRegion,
	IApiResultSearchAuthority,
	IApiResultSearchCompany,
	IApiResultSearchTender,
	IGetByIdCommand,
	IApiResultSector,
	IApiResultSectors,
	IApiResultStat,
...
export class ApiService {
	...
	post<T>(url: string, params: Object): Observable<T> {
		...
		return this.http.post<T>(this.actionCountryUrl + url, params, {
	...
	searchAuthority(
		params: ISearchCommand
	): Observable<IApiResultSearchAuthority> {
	...
	startDownload(params: IApiResultDownloadTenderSearch): void {
		window.location.href =
			this.actionCountryUrl + "download/id/" + params.data.id;
	}

	requestDownload(
		format: string,
		params: ISearchCommand
	): Observable<IApiResultDownloadTenderSearch> {
		return this.post<IApiResultDownloadTenderSearch>(
	...
	autocomplete(
		...
		field: string,
		search: string
	): Observable<IApiResultAutoComplete> {
		return this.post<IApiResultAutoComplete>("autocomplete", {
			entity,
			field,
			search,
		});
	...
	getPortalsStats(): Observable<IApiResultPortalsStats> {
		return this.http.get<IApiResultPortalsStats>(
			this.actionUrl +
				"portals/stats" +
				(this.config.locale ? "?lang=" + this.config.locale : "")
		);
	}

	getPortalMapData(): Observable<IApiResultGeoJSON> {
		return this.http.get<IApiResultGeoJSON>(
			this.actionUrl + "portals/geo.json"
		);
	}
	...
	ping(): Observable<IApiResultPing> {
		return this.http.get<IApiResultPing>(
			this.absUrl +
				"/ping" +
				(this.config.locale ? "?lang=" + this.config.locale : "")
		);
	...
	getNutsMap(level: number): Observable<IApiResultGeoJSON> {
		return this.http.get<IApiResultGeoJSON>(
			this.absUrl + "/data/nuts/nuts_20M_lvl" + level + ".geo.json"
		);
	}
}
