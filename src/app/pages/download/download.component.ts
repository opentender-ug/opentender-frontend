...
export class DownloadPage implements OnInit {
	...
	public getOcdsSize(country_code) {
		let d = this.downloads_ocsd.find(
			(x) => x.country == country_code.toUpperCase()
		);
	...
	public getOcdsFilename(country_code) {
		let d = this.downloads_ocsd.find(
			(x) => x.country == country_code.toUpperCase()
		);
	...
	constructor(
		...
		this.current = {
			...
			name: this.i18n.getPortalName(
				current_id.toUpperCase(),
				this.i18n.expandCountry(current_id)
			),
			formats: {},
		};
	}
	...
	refreshOCDSDownloads() {
		let sub = this.api.getOCDSDownloads().subscribe(
			(data: Array<IDownloadOCDS>) => {
				this.downloads_ocsd = data
					.map((download) => {
						let country_code = download.filename.replace(
							"_ocds_data.json.tar.gz",
							""
						);
						...
						return {
							name: this.i18n.getPortalName(
								country_code,
								this.i18n.expandCountry(country_code)
							),
				...
				this.downloads = data
					...
					.map((download) => {
						...
						if (result.country === "eu") {
							...
							result.name = this.i18n.getPortalName(
								result.country.toUpperCase(),
								this.i18n.expandCountry(result.country)
							);
						}
						if (
							result.country.toUpperCase() === this.current.id &&
							result.count > 0
						) {
